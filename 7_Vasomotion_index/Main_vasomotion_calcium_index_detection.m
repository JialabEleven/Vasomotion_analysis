...
buildingDir = fullfile([cur_p '7_Vasomotion_index\Raw_data\Test_figure_diameter_radius_merge.xls']);
savepath = fullfile([cur_p '7_Vasomotion_index\data_deal_diameter_radius_calcium\']);
...
% output_R_distance=[green_red_R;pval1;green_red_R_raw;pval2]';
...
A = [{'R','R0','deltaR/R0','peak_position','delta_diameter(radius,calcium)_value','peak_deltaD/D0(R/R0,F/F0)'}; num2cell(output)];
xlswrite([savepath,'single_data'],A,['cell_',num2str(iii),'']);
total_data=[frequency' average_amplitude' interval_mean' interval_SD' mean_deltaD' mean_diameter'];
B = [{'Frequency(Hz)','Average_deltaR/R0','interval_mean(s)','interval_SD','mean_deltaD(R,F)' 'mean_diameter(radius,calcium)'}; num2cell(total_data)];
